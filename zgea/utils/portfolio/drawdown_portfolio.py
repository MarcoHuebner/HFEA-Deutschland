...
class DrawdownPortfolio():
    @typechecked()
    def __init__(
            self,
            setup: Dict[str, Dict[str, Union[str, float, dict]]],
        ...
        """
        Initialize drawdown portfolio class, which handles cash based on current market state.
        ...
        total_alloc = sum([v['dist'] for v in setup.values()])
        assert total_alloc <= 100, f"Your Portfolio has an allocation of {sum([d['dist'] for d in setup.values()])}%"

        # assign cash as discrepancy to invested assets
        #setup["cash"] = dict(dist=100-total_alloc, ref_asset="cash")

        for name, v in setup.items():
            assert 'dist' in v, "Every asset needs a key 'dist'!"
            
            # initialize change of distribution threshold (normal portfolio)
            if 'down' not in v:
                v['down'] = {"change": 100, "exposure": 0}
                v['ref_asset'] = name
            assert 'ref_asset' in v, "Every asset needs a key 'ref_asset'!"
            assert (v['down']['change'] <= 100) and (v['down']['change'] >= 0), "Strategy change has invalid range!"
            assert (v['down']['exposure'] <= (100 - total_alloc)), "Margin larger than available cash position!"
    ...
    def backtest(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # create last high and last low DataFrame
        last_high = pd.DataFrame(columns=asset_names, index=data.index)
        new_highs = pd.DataFrame(columns=asset_names)
        last_low = pd.DataFrame(columns=asset_names, index=data.index)
        self._values = {}

        # DOCU: why is this overwritten/ re-initialized?
        ...
        for name, setup in self._setup.items():
            ...
            asset_price = data.loc[data.iloc[0].name, name]
            assets[name] = Asset(name, detailed_output = False)
            assets[name].buy(value_to_buy/asset_price, asset_price)
            # calculate last high for all given (asset) names
            last_high[name] = data[self._setup[name]['ref_asset']].cummax()
            # get dates of new highs
            new_highs[name] = last_high[name].drop_duplicates()
            # calculate last low since new high
            for i, val in enumerate(new_highs.index.tolist()):
                if i == 0:
                    continue
                
                date_start = new_highs.index[i-1]
                date_end = val

                last_low[name].loc[date_start:date_end] = data[self._setup[name]['ref_asset']].loc[date_start:date_end].cummin()
            last_low[name].loc[date_end:] = data[self._setup[name]['ref_asset']].loc[date_end:].cummin()
        ...
        for i in portfolio_values.index:
            ...
            for name in asset_names:
                ...
                # for all assets, compare the ref_asset price to the previously calculated last high and low
                compare_asset_price = data.loc[i, self._setup[name]['ref_asset']]

                # increase exposure if down and portfolio so far in normal market (redistribute ONLY ONCE)
                if self._state == "normal":
                    if (last_high.loc[i, name] - compare_asset_price) / last_high.loc[i, name] > (self._setup[name]["down"]["change"] / 100):
                        self._setup[name]['dist'] += self._setup[name]['down']['exposure']
                        self._state = "down"

                # decrease exposure if up
                elif self._state == "down":
                    if (compare_asset_price - last_low.loc[i, name]) / last_low.loc[i, name] > (self._setup[name]["down"]["change"] / 100):
                        self._setup[name]['dist'] -= self._setup[name]['down']['exposure']
                        self._state = "normal"                

                # update portfolio value (and do regular re-balancing)
        ...
        for name, _ in self._setup.items():
            n = data[name]
            self._details_memory['chart'][name + '_last_high'] = last_high[name]
            self._details_memory['chart'][name + '_last_low'] = last_low[name]
            self._details_memory['chart'][name + '_ref_asset'] = data[self._setup[name]['ref_asset']]
